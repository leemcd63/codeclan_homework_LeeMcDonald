...
```{r}
# Use str_detect to find if Thought Police is ever mentioned, inclusive of capitalisation
whole_text %>%
  str_detect("[tT]hought [pP]olice")

# Can also use regex(ignore_case)
whole_text %>%
  str_detect(regex("thought police", ignore_case = TRUE))
```


### Count how often Big Brother is mentioned
```{r}
# Use function str_count to find Big Brother whether capitalised or not
# This works but could return other results e.g. "bad bicycle"
whole_text %>%
  str_count("[Bb][a-zA-Z]{2} [Bb][a-zA-Z]{6}")

# More thorough but accurate
whole_text %>%
  str_count("[Bb][Ii][Gg] [Bb][Rr][Oo][Tt][Hh][Ee][Rr]")

# Or, using the regex() function with ignore_case argument
whole_text %>%
  str_count(regex("big brother", ignore_case = TRUE))
...
```{r}
# use str_detect and regex(ignore_case) to search for mention of julia
whole_text %>%
  str_detect(regex("julia", ignore_case = TRUE))
```
...
```{r}
# First looking for a word beginning with a capital letter (the month), a space, followed by 1 or more numbers with two letters directly after, a comma then another space, than a number with 4 digits.
whole_text %>%
  str_extract("[A-Z][a-z]+ [0-9]+[a-z]{2}, [0-9]{4}")
```
...
whole_text %>%
  str_replace_all("[wW]inston", "REDACTED") %>%
  str_detect("[wW]inston")

# adjusted after finding out about the ignore_case argument within regex()
whole_text %>%
  str_replace_all(regex("winston", ignore_case = TRUE), "REDACTED") %>%
  str_detect(regex("winston", ignore_case = TRUE))
```
...
```{r}
# as we are looking to match case when replacing it's better to specifically replacethe word "gin" in different cases rather than ignore_case. 

# str_extract_all() for gin, ignoring case
whole_text %>%
  str_extract_all(regex("gin", ignore_case = TRUE))

# 9 instances returned. However we must take care to add a space on the left side as words like "begin" could be affected. 
whole_text %>%
  str_extract_all(regex(" gin", ignore_case = TRUE))

# Now go ahead and str_replace_all() for each capitalisation that appears in str_replace_all
no_more_gin <- whole_text %>%
  str_replace_all(" gin", " vodka") %>%
  str_replace_all(" GIN", " VODKA")

# Check it worked
no_more_gin %>%
  str_detect(regex(" gin", ignore_case = TRUE))
```
...
```{r}
# First lookfor instances of Ministry, departments and "mini" ignoring case

whole_text %>%
  str_extract_all(regex("ministry", ignore_case = TRUE))

whole_text %>%
  str_extract_all(regex("department", ignore_case = TRUE))

# (?!) expression means "not followed by", so looking for all instances of "Mini" not followed by "stry"
whole_text %>%
  str_extract_all(regex("Mini(?!stry)", ignore_case = TRUE))

# use str_replace_all to replace these, paying close attention to the order. As we are replacing "Department" with "Section" we should do this before replacing "Ministry" with "Department"
no_more_ministry <- whole_text %>%
